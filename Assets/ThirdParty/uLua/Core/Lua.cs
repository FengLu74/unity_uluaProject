...
namespace LuaInterface
{
    ...
    public class LuaState : IDisposable
    {
        ...
        private byte[] ResLoadAsset(string path)
        {
            byte[] text = null;
            ...
            path = path.Replace('.', '/');
            path = "Lua/" + path;
            Debug.Log("ResLoadAsset:path-->" + path);
            text = Resources.Load<TextAsset>(path).bytes;
            Debug.Log("ResLoadAsset:text.Length-->" + text.Length);
            return text;
        }
        /*
