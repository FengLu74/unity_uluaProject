...
public class UIToggleWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("invertSpriteState", get_invertSpriteState, set_invertSpriteState),
	...
	static int get_invertSpriteState(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name invertSpriteState");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index invertSpriteState on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.invertSpriteState);
		return 1;
	}
	...
	static int set_invertSpriteState(IntPtr L)
	{
		...
		obj.invertSpriteState = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
