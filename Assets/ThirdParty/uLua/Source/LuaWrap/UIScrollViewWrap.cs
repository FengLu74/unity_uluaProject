...
public class UIScrollViewWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("constrainOnDrag", get_constrainOnDrag, set_constrainOnDrag),
	...
	static int get_constrainOnDrag(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name constrainOnDrag");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index constrainOnDrag on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.constrainOnDrag);
		return 1;
	}
	...
	static int set_constrainOnDrag(IntPtr L)
	{
		...
		obj.constrainOnDrag = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
