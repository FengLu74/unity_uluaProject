...
public class UICameraWrap
{
	public static void Register(IntPtr L)
	{
		LuaMethod[] regs = new LuaMethod[]
		{
			...
			new LuaMethod("ClearData", ClearData),
			...
			new LuaMethod("CancelNextTooltip", CancelNextTooltip),
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("GetMouse", get_GetMouse, set_GetMouse),
			new LuaField("GetTouch", get_GetTouch, set_GetTouch),
			new LuaField("RemoveTouch", get_RemoveTouch, set_RemoveTouch),
			...
			new LuaField("processEventsIn", get_processEventsIn, set_processEventsIn),
			...
			new LuaField("ignoreAllEvents", get_ignoreAllEvents, set_ignoreAllEvents),
			...
			new LuaField("lastWorldRay", get_lastWorldRay, set_lastWorldRay),
			...
			new LuaField("mouse0", get_mouse0, null),
			new LuaField("mouse1", get_mouse1, null),
			new LuaField("mouse2", get_mouse2, null),
			...
			new LuaField("uiHasFocus", get_uiHasFocus, null),
	...
	{
		LuaScriptMgr.Push(L, UICamera.GetMouse);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.GetTouch);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.RemoveTouch);
		return 1;
	}
	...
	static int get_processEventsIn(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name processEventsIn");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index processEventsIn on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.processEventsIn);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.ignoreAllEvents);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.lastWorldRay);
		return 1;
	}
	...
	{
		LuaScriptMgr.PushObject(L, UICamera.mouse0);
		return 1;
	}
	...
	{
		LuaScriptMgr.PushObject(L, UICamera.mouse1);
		return 1;
	}
	...
	{
		LuaScriptMgr.PushObject(L, UICamera.mouse2);
		return 1;
	}
	...
	{
		LuaScriptMgr.Push(L, UICamera.uiHasFocus);
		return 1;
	}
	...
	static int set_GetMouse(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.GetMouse = (UICamera.GetMouseDelegate)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.GetMouseDelegate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.GetMouse = (param0) =>
			{
				int top = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				func.PCall(top, 1);
	...
	static int set_GetTouch(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.GetTouch = (UICamera.GetTouchDelegate)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.GetTouchDelegate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.GetTouch = (param0, param1) =>
			{
				int top = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
	...
	static int set_RemoveTouch(IntPtr L)
	{
		...
		if (funcType != LuaTypes.LUA_TFUNCTION)
		{
			UICamera.RemoveTouch = (UICamera.RemoveTouchDelegate)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.RemoveTouchDelegate));
		}
		else
		{
			LuaFunction func = LuaScriptMgr.ToLuaFunction(L, 3);
			UICamera.RemoveTouch = (param0) =>
			{
				int top = func.BeginPCall();
				LuaScriptMgr.Push(L, param0);
				func.PCall(top, 1);
	...
	static int set_processEventsIn(IntPtr L)
	{
		...
		obj.processEventsIn = (UICamera.ProcessEventsIn)LuaScriptMgr.GetNetObject(L, 3, typeof(UICamera.ProcessEventsIn));
		return 0;
	}
	...
	{
		UICamera.ignoreAllEvents = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
	...
	{
		UICamera.lastWorldRay = LuaScriptMgr.GetRay(L, 3);
		return 0;
	}
