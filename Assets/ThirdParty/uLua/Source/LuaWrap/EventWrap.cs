...
public class EventWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("isScrollWheel", get_isScrollWheel, null),
			...
			new LuaField("displayIndex", get_displayIndex, set_displayIndex),
		};
	...
	static int _CreateEvent(IntPtr L)
	{
		...
		else if (count == 1 && LuaScriptMgr.CheckTypes(L, 1, typeof(Event)))
		{
	...
	static int get_isScrollWheel(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name isScrollWheel");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index isScrollWheel on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.isScrollWheel);
		return 1;
	}
	...
	static int get_displayIndex(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name displayIndex");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index displayIndex on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.displayIndex);
		return 1;
	}
	...
	static int set_displayIndex(IntPtr L)
	{
		...
		obj.displayIndex = (int)LuaScriptMgr.GetNumber(L, 3);
		return 0;
	}
