...
public class UITweenerWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("useFixedUpdate", get_useFixedUpdate, set_useFixedUpdate),
	...
	static int get_useFixedUpdate(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name useFixedUpdate");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index useFixedUpdate on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.useFixedUpdate);
		return 1;
	}
	...
	static int set_useFixedUpdate(IntPtr L)
	{
		...
		obj.useFixedUpdate = LuaScriptMgr.GetBoolean(L, 3);
		return 0;
	}
