...
public class LuaScriptMgr
{
    ...
    private int m_iFileNum = 300;//PC端默认为300,在手机平台读取配置信息
    /// <summary>
    /// panel文件夹下loading进度,文件过多,需要分针处理进度,采用20个文件处理进度
    /// </summary>
    private int m_iPanelDoFileLoading = 0;
    /// <summary>
    /// panel文件基础数量20个
    /// </summary>
    private int m_iPanelFileBaseNum = 20;

    //lua文件夹目录
    public static string LuaFileFolderPath = "/Lua/";
    //lua/Excel文件夹目录
    public static string ExcelTableFolderPath = "Excel/";
    //lua/Utility文件夹目录
    public static string UtilityLuaFileFolderPath = "Utility/";
    //lua/Variable文件夹目录
    public static string VariableLuaFileFolderPath = "Variable/";
    //lua/Manager文件夹目录
    public static string ManagerLuaFileFolderPath = "Manager/";
    //lua/Protocol文件夹目录
    public static string ProtocolLuaFileFolderPath = "Protocol/";
    //lua/Panel文件夹目录
    public static string PanelLuaFileFolderPath = "Panel/";
    //lua/Configs文件夹目录
    public static string ConfigLuaFileFolderPath = "Configs/";
    ...
    /// asssetBundle资源管理器
    /// </summary>
    //private CAssetManager m_AssetBundleMng = null;
    ...
    public LuaScriptMgr()
    {
        ...
        //m_AssetBundleMng = Ioo.AssetBundleManager;
    ...
    private Action<bool> nextDoFunc = null;
    public void Start(Action<bool> func)
    {
        nextDoFunc = func;
        MonoTools.Instance.StartCoroutine(OnBundleLoaded());
    }
    public void Start()
    {
        ...
        MonoTools.Instance.StartCoroutine(OnBundleLoaded());
        //m_AssetBundleMng.StartCoroutine(OnBundleLoaded());
    ...
    private IEnumerator OnBundleLoaded()
    {
        ...
        DoFile("Configs/AllLuaFileNameConfig.lua");
        LuaTable luaTable;
        luaTable = GetLuaTable("AllLuaFileNameConfig.FileNum");
        foreach (DictionaryEntry entry in luaTable)
        {
            if (false == int.TryParse(entry.Value.ToString(), out m_iFileNum))
                Logger.LogError("m_iFileNum::::read fail");
            else
                Logger.Log("OnBundleLoaded:::m_iFileNum:::" + m_iFileNum);
        }
#endif

#if UNITY_EDITOR
        string LuaFilePath = Application.dataPath + LuaFileFolderPath;
        DirectoryInfo folder;
        //DoFile Utility文件夹下的所有文件
        string UtilityLuaFilePath = LuaFilePath + UtilityLuaFileFolderPath;
        ...
        {
            DoFile(UtilityLuaFileFolderPath + file.Name);
            fileCount++;
            if (fileCount >= 5)
            {
                fileCount = 0;
        ...
        yield return new WaitForEndOfFrame();
#else 
        luaTable = GetLuaTable("AllLuaFileNameConfig.UtilityLuaFileTable");
        foreach (DictionaryEntry entry in luaTable)
        {
            DoFile(entry.Value.ToString());
        }
        yield return new WaitForEndOfFrame();
#endif
        //InitLayers(lua.L);
...
#if UNITY_EDITOR
        //DoFile Variable文件夹下的所有文件
        
        string VariableLuaFilePath = LuaFilePath + VariableLuaFileFolderPath;
        ...
        foreach (FileInfo file in folder.GetFiles("*.lua"))
        {
            DoFile(VariableLuaFileFolderPath + file.Name);
        }
        yield return new WaitForEndOfFrame();
		/*
        //DoFile Excel文件夹下的所有文件
        string ExcelLuaFilePath = LuaFilePath + ExcelTableFolderPath;
        ...
        foreach (FileInfo file in folder.GetFiles("*.lua"))
        {
            DoFile(ExcelTableFolderPath + file.Name);
        }
		*/
        //DoFile Manager文件夹下的所有文件
        string ManagerLuaFilePath = LuaFilePath + ManagerLuaFileFolderPath;
        ...
        foreach (FileInfo file in folder.GetFiles("*.lua"))
        {
            DoFile(ManagerLuaFileFolderPath + file.Name);
        }
        yield return new WaitForEndOfFrame();
        //DoFile Protocol文件夹下的所有文件
        string ProtocolLuaFilePath = LuaFilePath + ProtocolLuaFileFolderPath;
        ...
        foreach (FileInfo file in folder.GetFiles("*.lua"))
        {
            DoFile(ProtocolLuaFileFolderPath + file.Name);
            CallLuaFunction(file.Name.Replace(".lua", "") + ".RegisterProtocols");
        }
        yield return new WaitForEndOfFrame();
        //DoFile Panel文件夹下的所有文件
        string PanelLuaFilePath = LuaFilePath + PanelLuaFileFolderPath;
        ...
        foreach (FileInfo file in folder.GetFiles("*.lua"))
        {
            if (0 == (m_iPanelDoFileLoading++ % m_iPanelFileBaseNum))
                yield return new WaitForEndOfFrame();
            DoFile(PanelLuaFileFolderPath + file.Name);
        }
        yield return new WaitForEndOfFrame();
#else

        //luaTable = GetLuaTable("AllLuaFileNameConfig.ExcelLuaFileTable");
        //foreach (DictionaryEntry entry in luaTable)
        //{
        //    DoFile(entry.Value.ToString());
        //}
        ...
        luaTable = GetLuaTable("AllLuaFileNameConfig.VariableLuaFileTable");
        foreach (DictionaryEntry entry in luaTable)
        {
            DoFile(entry.Value.ToString());
        }
        ...
        luaTable = GetLuaTable("AllLuaFileNameConfig.ManagerLuaFileTable");
        foreach (DictionaryEntry entry in luaTable)
        {
            DoFile(entry.Value.ToString());
        }
        yield return new WaitForEndOfFrame();
        luaTable = GetLuaTable("AllLuaFileNameConfig.ProtocolLuaFileTable");
        string[] ProtocolfuncName;
        foreach (DictionaryEntry entry in luaTable)
        {
            DoFile(entry.Value.ToString());
            ProtocolfuncName = entry.Value.ToString().Split('/');
            CallLuaFunction(ProtocolfuncName[ProtocolfuncName.Length - 1].Replace(".lua", "") + ".RegisterProtocols");
        }
        yield return new WaitForEndOfFrame();
        luaTable = GetLuaTable("AllLuaFileNameConfig.PanelLuaFileTable");
        foreach (DictionaryEntry entry in luaTable)
        {
            if (0 == (m_iPanelDoFileLoading++ % m_iPanelFileBaseNum))
                yield return new WaitForEndOfFrame();
        ...
        yield return new WaitForEndOfFrame();
#endif
        //DoFile Configs文件夹下的config文件
        //DoFile("Configs/Config");
        ...
        //CallLuaFunction("getTableData");
        //DataTableManager.Instance.ReadAllTable();
        if (nextDoFunc != null)
        {
            nextDoFunc(true);
        }
    ...
    public object[] DoFile(string fileName)
    {
        ...
        {
            //if (Ioo.GameManager.UISliderLoadProgress != null)
            //    Debug.Log("Start dofile fileName:::" + fileName + "::UISliderLoadProgress:::" + Ioo.GameManager.UISliderLoadProgress.value);
            //else
            //    Debug.Log("Start dofile fileName:::" + fileName);
#if UNITY_EDITOR
            int index = fileName.LastIndexOf("/") + 1;
            string tableName = fileName.Substring(index).Replace(".lua", "");
            if (!string.IsNullOrEmpty(tableName))
            {
                var keys = new List<string>();
                foreach (var pair in dict)
                {
                    if (pair.Key.StartsWith(tableName + "."))
                        keys.Add(pair.Key);
                }
...
#endif
            if (fileList.Count <= m_iFileNum)//避免进度条回退
                //EventDispatcher.TriggerEvent<float>(Events.AssetLoadEvent.Load, 0.1f + fileList.Count * 0.1f / m_iFileNum);
            return lua.DoFile(fileName, null);
        }
        else
        {
            //Debug.Log("already dofile which name is " + fileName);
        }
...
#if UNITY_EDITOR
        //LuaScriptDebug.Instance.EndCall(name);
#endif
        return results;
    }
    ...
    public LuaFunction GetLuaFunction(string name)
    {
        ...
        if (!dict.TryGetValue(name, out func))
        {
            ...
            {
                //UnityEngine.Debug.LogError(string.Format("Lua function {0} not exists", name));
            }
    ...
    public byte[] Loader(string name)
    {
        ...
        //    int index = name.LastIndexOf(".lua");
        //    if (index > 0)
        //    {
        ...
        //    }
        //    path = path.Replace('.', '/');
        //    path = "AllPrefabs/uLua/Lua/" + path;
        //    //Debug.Log("Loader::Start:::path is " + path);
        //    TextAsset asset = m_AssetBundleMng.Load<TextAsset>(path);
        //    if (asset != null)
        //    {
        //        //Debug.LogError("asset : {0}", asset.text);
        //        buffer = asset.bytes;
        //        //Debug.Log("Loader::success:::path is " + path + ":::buffer:::" + buffer.Length);
        //    }
        //    else
        //    {
        //        Debug.LogError("Loader fail asset is null :::::: path is " + path);
        //    }
        //    m_AssetBundleMng.UnloadAsset(path, asset);
        //}
        //fileList.Add(name);
        return buffer;
    }
    ...
    {
        int reference = LuaDLL.luaL_ref(lua.L, LuaIndexes.LUA_REGISTRYINDEX);
        return new LuaTable(reference, lua.L);
    }
