...
public class UIPanel : UIRect
{
    ...
    /// Panel's GrayScale
    /// </summary>
    private bool mIsGrayScale = false;
	...
	void UpdateSelf ()
	{
		...
		{
			mUpdateScroll = false;
			//注释掉 我们项目没有使用UIScrollView, 避免GetComponent产生GC
			//UIScrollView sv = GetComponent<UIScrollView>();
		...
		for (int i = 0; i < widgets.Count; ++i)
		{
			...
			if (w.isVisible && w.hasVertices)
			{
				...
				if (mat != null || sdr != null || tex != null)
				{
					...
					++count;
                    if (generateNormals) w.WriteToBuffers(dc.verts, dc.uvs, dc.cols, dc.norms, dc.tans, generateUV2 ? dc.uv2 : null,dc.uv3);
                    /// 此方法被重写了 刘洋 2016年66月30日15:12:30
                    else w.WriteToBuffers(dc.verts, dc.uvs, dc.cols, dc.norms, dc.tans, generateUV2 ? dc.uv2 : null, dc.uv3);
