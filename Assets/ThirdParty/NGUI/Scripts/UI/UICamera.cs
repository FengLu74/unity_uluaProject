...
public class UICamera : MonoBehaviour
{
    ...
    /// <summary>
    /// 切换后台清理数据--liuyingwei添加2017-5-8 13:56:23
    /// </summary>
    static public void ClearData()
    {
        activeTouches.Clear();
        mTouchIDs.Clear();
        for (int Index = 0; Index < mMouse.Length; ++Index)
        {
            if (mMouse[Index] != null)
            {
                mMouse[Index].last = null;
                mMouse[Index].current = null;
                mMouse[Index].dragged = null;
                mMouse[Index].pressed = null;
            }
        }
        if (controller != null)
        {
