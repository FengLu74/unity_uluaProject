...
public class UILabel : UIWidget
{
    ...
    /// Retrieve the URL directly below the specified world-space position.
    /// <范晓辉修改>
    /// Vector3/Vector2在lua中用纯Lua实现, 调用C#函数时在C#端再将luatable转换成Vector3/Vector2
    /// 此时在调用GetUrlAtPosition函数时, 不能根据参数类型来区分是Vector3还是Vector2
    /// 因此添加GetUrlAtWorldPosition方法, 供Lua调用,来获取世界坐标下的url值
    /// </summary>
    public string GetUrlAtWorldPosition(Vector3 worldPos) { return GetUrlAtPosition(worldPos); }
    ...
    /// Vector3/Vector2在lua中用纯Lua实现, 调用C#函数时在C#端再将luatable转换成Vector3/Vector2
    /// 此时在调用GetUrlAtPosition函数时, 不能根据参数类型来区分是Vector3还是Vector2
    /// 因此添加GetUrlAtLocalPosition方法, 供Lua调用,来获取本地坐标下的url值
    /// </summary>
    public string GetUrlAtLocalPosition(Vector2 localPos) { return GetUrlAtPosition(localPos); }
