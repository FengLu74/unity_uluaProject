...
public class UICenterOnChild : MonoBehaviour
{
    ...
    /// 移动偏移比率
    /// </summary>
    public float MoveRatio = 0.15f;
    /// <summary>
    /// 是否打开移动便宜比率控制
    /// </summary>
    public bool isOpenMoveRatio = false;
    ...
    public void Recenter()
    {
        ...
        if (isOpenMoveRatio)
        {
            if (UIScrollView.Movement.Vertical == mScrollView.movement)
            {
                if (UICamera.currentTouch != null && UICamera.currentTouch.totalDelta.y > 0)
                    pickingPoint.y = corners[0].y + Mathf.Abs(corners[2].y - corners[0].y) * MoveRatio;
                else
                    pickingPoint.y = corners[0].y + Mathf.Abs(corners[2].y - corners[0].y) * (1 - MoveRatio);
            }
            else if (UIScrollView.Movement.Horizontal == mScrollView.movement)
            {
                if (UICamera.currentTouch != null && UICamera.currentTouch.totalDelta.x > 0)
                    pickingPoint.x = corners[0].x + Mathf.Abs(corners[2].x - corners[0].x) * MoveRatio;
                else
                    pickingPoint.x = corners[0].x + Mathf.Abs(corners[2].x - corners[0].x) * (1 - MoveRatio);
            }
        }
        //liuyingwei 修改内容 结束
    ...
    {
        //Debug.LogError("CenterOn::::target::::" + target);
