...
public class UIPanelInspector : UIRectEditor
{
	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (NGUIEditorTools.DrawHeader("Advanced Options"))
		{
            ...
            GUILayout.BeginHorizontal();
            bool isGrayScale = EditorGUILayout.Toggle("IsGrayScale", mPanel.isGrayScale, GUILayout.Width(100f));
            GUILayout.Label("Panel is Gray", GUILayout.MinWidth(20f));
            GUILayout.EndHorizontal();
            if (mPanel.isGrayScale != isGrayScale)
            {
                mPanel.isGrayScale = isGrayScale;
                mPanel.RebuildAllDrawCalls();
                EditorUtility.SetDirty(mPanel);
            }
