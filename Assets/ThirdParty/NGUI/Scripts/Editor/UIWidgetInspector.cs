...
public class UIWidgetInspector : UIRectEditor
{
	...
	{
        float brightnessAmount = EditorGUILayout.Slider("亮度", w.brightnessAmount, 0f, 2f);
        float saturationAmount = EditorGUILayout.Slider("饱和度", w.saturationAmount, 0f, 2f);
        float contrastAmount = EditorGUILayout.Slider("对比度", w.contrastAmount, 0f, 2f);
        if (brightnessAmount != w.brightnessAmount)
        {
            NGUIEditorTools.RegisterUndo("BrightnessAmount Change", w);
            w.brightnessAmount = brightnessAmount;
        }
        if (saturationAmount != w.saturationAmount)
        {
            NGUIEditorTools.RegisterUndo("saturationAmount Change", w);
            w.saturationAmount = saturationAmount;
        }
        if (contrastAmount != w.contrastAmount)
        {
            NGUIEditorTools.RegisterUndo("contrastAmount Change", w);
            w.contrastAmount = contrastAmount;
        }
		if (drawColor)
		{
			DrawColor(so, w);
