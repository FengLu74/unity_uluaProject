...
public abstract class UIBasicSprite : UIWidget
{
	public enum Type
	{
		...
		Filled,
		Advanced,
        RadialGradient,//2017.3.7刘洋添加径向渐变效果
        ColorGradient,//2017.4.27刘洋添加渐变效果
        RadialClipping,//圆形遮罩效果
	}
    ...
    [HideInInspector][SerializeField] protected ColorChangeDirection mColorChangeDirection = ColorChangeDirection.TopToBottom;
    ...
    /// <summary>
    /// 径向渐变圆心位置 ——2017.3.7刘洋添加
    /// </summary>
    public Vector2 radialCircleCenterPos = new Vector2();
    /// <summary>
    /// 径向渐变直径 ——2017.3.7刘洋添加
    /// </summary>
    public int radialCircleRadius = 0;
...
#region Fill Functions
	...
	protected void Fill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, Rect outer, Rect inner)
	{
		...
		switch (type)
		{
            ...
            RadialGradientFill(verts, uvs, cols);
			break;

            case Type.ColorGradient:
            ColorGradientFill(verts, uvs, cols);
            break;

            case Type.RadialClipping:
            RadialClippingFill(verts, uvs, cols);
            break;
		}

        if (type != Type.RadialGradient && type != Type.ColorGradient && type != Type.RadialClipping)
        {
            geometry.ApplyRadialGradient(UIGeometry.WidgetRenderType.Normal, 0, 0, 0);
        }
	}

    /// <summary>
    /// 填充mesh信息，在RadialGradient的模式下
    /// </summary>
    /// <param name="verts">顶点信息</param>
    /// <param name="uvs">uv信息</param>
    /// <param name="cols">颜色信息</param>
    void RadialGradientFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        ...
        cols.Add(gc);
        cols.Add(gc);
        float newPosX = (u.x + u.z) / 2 + (u.z - u.x) / mWidth * radialCircleCenterPos.x;
        float newPosY = (u.w + u.y) / 2 + (u.w - u.y) / mHeight * radialCircleCenterPos.y;
        float newRadius = (u.w - u.y) / mHeight * radialCircleRadius;
        if (mType == Type.RadialGradient)
        {
            geometry.ApplyRadialGradient(UIGeometry.WidgetRenderType.RadialGradient, newPosX, newPosY, newRadius);
        }
        else
            geometry.ApplyRadialGradient(UIGeometry.WidgetRenderType.Normal, newPosX, newPosY, newRadius);
    }

    /// <summary>
    /// 填充mesh信息，在ColorGradient的模式下
    /// </summary>
    /// <param name="verts">顶点信息</param>
    /// <param name="uvs">uv信息</param>
    /// <param name="cols">颜色信息</param>
    void ColorGradientFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color gc = drawingColor;
        ...
        uvs.Add(new Vector2(u.z, u.w));
        uvs.Add(new Vector2(u.z, u.y));
        switch (mColorChangeDirection)
        {
            case ColorChangeDirection.TopToBottom:
                ...
                break;
            case ColorChangeDirection.LeftToRight:
                AddVertexColours(cols, ref gc, 1, 2);
                AddVertexColours(cols, ref gc, 1, 2);
                AddVertexColours(cols, ref gc, 2, 1);
                AddVertexColours(cols, ref gc, 2, 1);
                break;
            case ColorChangeDirection.RightToLeft:
                AddVertexColours(cols, ref gc, 1, 1);
                AddVertexColours(cols, ref gc, 1, 1);
                AddVertexColours(cols, ref gc, 2, 2);
                AddVertexColours(cols, ref gc, 2, 2);
                break;
            case ColorChangeDirection.BottomToTop:
                AddVertexColours(cols, ref gc, 1, 2);
                AddVertexColours(cols, ref gc, 1, 1);
                AddVertexColours(cols, ref gc, 2, 1);
                AddVertexColours(cols, ref gc, 2, 2);
        ...
        geometry.ApplyRadialGradient(UIGeometry.WidgetRenderType.ColorGradient, 0, 0, 0);
    }
    ...
    void RadialClippingFill(List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
    {
        Vector4 v = drawingDimensions;
        Vector4 u = drawingUVs;
        Color gc = drawingColor;
        ...
        float newPosY = (u.w + u.y) / 2 + (u.w - u.y) / mHeight * radialCircleCenterPos.y;
        float newRadius = (u.w - u.y) / mHeight * radialCircleRadius;
        geometry.ApplyRadialGradient(UIGeometry.WidgetRenderType.RadialClipping, newPosX, newPosY, newRadius);
    }
				...
				else
				{
                    switch (mColorChangeDirection)
                    {
                        case ColorChangeDirection.TopToBottom:
                            ...
                            break;
                        case ColorChangeDirection.LeftToRight:
                            AddVertexColours(cols, ref gc, x2, 3-x);
                            AddVertexColours(cols, ref gc, x2, 3-x);
                            AddVertexColours(cols, ref gc, x, 3-x2);
                            AddVertexColours(cols, ref gc, x2, 3-x2);
                            break;
                        case ColorChangeDirection.RightToLeft:
                            AddVertexColours(cols, ref gc, x, x);
                            AddVertexColours(cols, ref gc, x, x);
                            AddVertexColours(cols, ref gc, x2, x2);
                            AddVertexColours(cols, ref gc, x2, x2);
                            break;
                        case ColorChangeDirection.BottomToTop:
                            AddVertexColours(cols, ref gc, x, 3-y);
                            AddVertexColours(cols, ref gc, x, 3-y2);
                            AddVertexColours(cols, ref gc, x2, 3-y2);
                            AddVertexColours(cols, ref gc, x2, 3-y);
                            break;
                        default:
                            break;
                    }
