...
public class UIWidget : UIRect
{
    ...
    /// 图片亮度值
    /// </summary>
    [HideInInspector][SerializeField]protected float mBrightnessAmount = 1.0f;
    public float brightnessAmount
    {
        get
        {
            return mBrightnessAmount;
        }
        set
        {
            ...
            if (mBrightnessAmount != curValue)
            {
                mBrightnessAmount = curValue;
                Invalidate(false);
            }
        }
    }
    ...
    /// 图片饱和度值
    /// </summary>
    [HideInInspector][SerializeField]protected float mSaturationAmount = 1.0f;
    public float saturationAmount
    {
        get
        {
            return mSaturationAmount;
        }
        set
        {
            float curValue = value;
            ...
            if (mSaturationAmount != curValue)
            {
                mSaturationAmount = curValue;
                Invalidate(false);
            }
        }
    }
    ...
    /// 图片对比度值
    /// </summary>
    [HideInInspector][SerializeField]protected float mContrastAmount = 1.0f;
    public float contrastAmount
    {
        get
        {
            return mContrastAmount;
        }
        set
        {
            float curValue = value;
            ...
            if (mContrastAmount != curValue)
            {
                mContrastAmount = curValue;
                Invalidate(false);
            }
        }
    }
		...
		if (mChanged)
		{
			if (mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			{
				...
				if (geometry.hasVertices)
				{
                    ...
                    geometry.ApplyImageDeal(mBrightnessAmount, mSaturationAmount, mContrastAmount,panel.isGrayScale);
					mMoved = false;
					mChanged = false;
					return true;
				}
	...
	{
		geometry.WriteToBuffers(v, u, c, n, t, u2, u3);
	}
